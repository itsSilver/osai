  ...
  <div>
    <b-overlay :show="show" rounded="sm">
      <o-table :data="dataTable" :bordered="true" :striped="true" mobile-cards paginated :per-page="perPage"
        ...
        <o-table-column field="rank" label="Rank" v-slot="props" searchable position="centered" sortable
          :visible="showRank">
          {{ props.row.rank }}
        </o-table-column>
        <o-table-column field="settore_riferimento" label="Reference sector" v-slot="props" searchable
          position="centered" sortable :visible="showReferenceSector">
        ...
        <o-table-column field="user_id" label="User Id" position="centered" v-slot="props" searchable sortable
          :visible="showUserId">
          {{ props.row.user_id }}
        </o-table-column>
        ...
        <o-table-column field="action" label="Action" position="centered" v-slot="props" width="180px">
          <b-button class="mx-1 view-btn" @click="pushRoute(`view/${props.row.id}`)"
            v-permission="'Can view soluzioni'">
          ...
          <b-button class="mx-1 delete-btn" @click="deleteDocument(props.row.id)" v-permission="'Can delete soluzioni'">
      ...
      <SeeImage v-if="showImage" :imageValue="imageValue" :noImageDisplay="noImageDisplay" @close="hideModal()" />
    </b-overlay>
  </div>
</template>
<script>
